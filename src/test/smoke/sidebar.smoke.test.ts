...
suite("sidebar smoke", () => {
  ...
  test("ganttee.explorer view is registered", async () => {
    ...
    assert.ok(all.includes("ganttee.explorer.focus"), "ganttee.explorer view not registered");
  });

  test("ganttee.refreshExplorer command is registered", async () => {
    const all = await vscode.commands.getCommands(true);
    assert.ok(all.includes("ganttee.refreshExplorer"), "refreshExplorer not registered");
  });
  ...
  test("refreshExplorer command executes without error after opening a fixture", async () => {
    ...
    await assert.doesNotReject(
      Promise.resolve(vscode.commands.executeCommand("ganttee.refreshExplorer")),
    );
  });
  ...
  test("revealEntity command is registered", async () => {
    const all = await vscode.commands.getCommands(true);
    assert.ok(all.includes("ganttee.revealEntity"), "ganttee.revealEntity not registered");
  });
});
