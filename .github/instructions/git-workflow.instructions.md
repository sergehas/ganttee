---
description: "Ganttee git workflow — Conventional Commits message format and gitflow branch naming. Use whenever writing a commit message, naming or creating a branch, or reviewing git history."
---

# Git Workflow

Ganttee uses **Conventional Commits** for every commit message and **gitflow**
for every branch name. These rules are enforced locally by git hooks
(`commitlint` + a branch-name check), so following them keeps commits and pushes
from being rejected.
...
Signal a breaking change with a `!` after the type/scope **and** a
...
(`added`), or exceed the length limits.
...
| `bugfix/*`      | Non-urgent fix branched from `develop`.        |
| `support/*`     | Long-term maintenance of an older release.     |

The segment after the prefix must be lowercase kebab-case, optionally with an
issue id:
...
bugfix/dependency-cycle-detection
```
...
Point git at the commit template once per clone so `git commit` (without `-m`)
...
`npm install` provisions the husky hooks that enforce these rules; no other
setup is required.
