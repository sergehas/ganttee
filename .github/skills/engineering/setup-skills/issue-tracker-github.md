...
- **Map**: a single issue labelled `wayfinder:map`, holding the Notes / Decisions-so-far / Fog body.
  `gh issue create --label wayfinder:map`. (On GitHub Projects (beta), native sub-issues or task
  lists may be used instead.)
- **Child ticket**: an issue carrying labels `wayfinder:<type>`
...
- **Blocking**: use GitHub's native issue dependencies
  (`gh issue edit <n> --add-project-v2-status <status>`), or a `Blocked by: #<n>, #<n>` line near
  the top. A ticket is unblocked when every issue it lists is closed.
